...
resource "ovh_dedicated_server" "{{ $.EnvName | replace "-" "_" }}_{{ $spec.Chassis | replace "." "_" }}_{{ $j }}" {
  ovh_subsidiary = data.ovh_me.account.ovh_subsidiary
  display_name   = "{{ $.EnvName }}-{{ $spec.Chassis | splitList "." | last }}-{{ printf "%03d" (add $j 1) }}"
  os             = "{{ $.Image }}"

  plan = [{
    plan_code    = "{{ $spec.OfferType }}"
    duration     = "P1M"
...
data "ovh_dedicated_server" "{{ $.EnvName | replace "-" "_" }}_{{ $spec.Chassis | replace "." "_" }}_{{ $j }}_data" {
  service_name = ovh_dedicated_server.{{ $.EnvName | replace "-" "_" }}_{{ $spec.Chassis | replace "." "_" }}_{{ $j }}.name
}

resource "ovh_vrack_dedicated_server_interface" "{{ $.EnvName | replace "-" "_" }}_{{ $spec.Chassis | replace "." "_" }}_{{ $j }}_vrack" {
  service_name = var.vrack_service_name
  interface_id = data.ovh_dedicated_server.{{ $.EnvName | replace "-" "_" }}_{{ $spec.Chassis | replace "." "_" }}_{{ $j }}_data.enabled_vrack_vnis[0]
}
{{- end }}
{{- end }}
{{- end }}
...
{{- else }}
      private_mac = ""
{{- end }}
      server_id   = tostring(ovh_dedicated_server.{{ $.EnvName | replace "-" "_" }}_{{ $spec.Chassis | replace "." "_" }}_{{ $j }}.server_id)
      zone        = "{{ $.Zone }}"
      region      = "{{ $.Region }}"
